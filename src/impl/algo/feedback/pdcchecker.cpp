...
void countMesConstr(const QMap<CoreId, QList<double>> &coreProblemDur, const QMap<CoreId, QList<std::pair<double, double>>> &coreProblemInt,
    ...
    foreach(CoreId c, coreProblemDur.keys()){
        ...
        for (int i = 0; i < durs.size(); i++){
            ...
            //по-хорошему: это должен быть список ј…ƒ»
...
//в  mesExtraConstr кладетс€ число, на которое необходимо уменьшить текщую длительность сообщени€
//всатвить проверку, текуща€ длительность сообщени€ > подсчитанное ограничение (иначе все бессмысленно)
...
// onlycheck = true - когда идет проверка в процессе ћ¬√, как только нашли проблему- сразу запрещаем этораспределение
bool PDCChecker::checkPDC(QMap <ObjectId, double> &messageDur, const QList<myTreeNode> &myList, ScheduleData *data, bool onlyCheck,
    ...
    //нужно помен€ть - сделать QMap<TaskId,QList<pair<double,double>>> - дл€ каждой задачи, чей период меньше I нужно считать несколько раз
